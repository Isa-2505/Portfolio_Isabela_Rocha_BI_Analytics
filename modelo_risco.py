...
ax1.set_ylabel('Real', fontsize=11, color=azul_serasa)
ax1.set_xlabel('Predito', fontsize=11, color=azul_serasa)
ax1.set_facecolor('#ffffff')

# Gráfico 2: Importância das Features
ax2 = plt.subplot(2, 2, 2)
cores_bar = [rosa_serasa, azul_serasa, '#FF9800']
ax2.barh(features, importancias, color=cores_bar, alpha=0.85, edgecolor='white', linewidth=2)
ax2.set_title('Importância das Features', fontsize=13, fontweight='bold', pad=15, color=azul_serasa)
ax2.set_xlabel('Importância', fontsize=11, color=azul_serasa)
ax2.grid(axis='x', alpha=0.3)
ax2.set_facecolor('#ffffff')
ax2.spines['top'].set_visible(False)
ax2.spines['right'].set_visible(False)

# Gráfico 3: Distribuição de Risco nos Dados
ax3 = plt.subplot(2, 2, 3)
riscos = dados['risco'].value_counts().sort_index()
cores_risco = ['#4CAF50', '#FFC107', '#F44336']
labels_risco = ['Baixo (0)', 'Médio (1)', 'Alto (2)']
ax3.bar(labels_risco, [riscos.get(i, 0) for i in [0, 1, 2]], 
...
ax3.set_ylabel('Quantidade de Clientes', fontsize=11, color=azul_serasa)
ax3.grid(axis='y', alpha=0.3)
ax3.set_facecolor('#ffffff')
ax3.spines['top'].set_visible(False)
ax3.spines['right'].set_visible(False)

# Gráfico 4: Acurácia e Previsões
...
cores_acc = [rosa_serasa, '#999999']
ax4.bar(categorias, valores, color=cores_acc, alpha=0.85, edgecolor='white', linewidth=2)
ax4.set_title('Precisão do Modelo', fontsize=13, fontweight='bold', pad=15, color=azul_serasa)
ax4.set_ylabel('Quantidade', fontsize=11, color=azul_serasa)
for i, (cat, val) in enumerate(zip(categorias, valores)):
    ax4.text(i, val + 0.1, str(val), ha='center', fontsize=12, fontweight='bold', color=azul_serasa)
